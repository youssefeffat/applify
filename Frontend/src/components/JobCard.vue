...
const application  = computed(() => store.savedJobs.find(j => String(j.job_id) === String(props.job.id)))
const appStatusRaw = computed(() => application.value?.status || '') // Original case for badge
const appStatus    = computed(() => appStatusRaw.value.toLowerCase())
...
const scoreColor = computed(() => {
  ...
  if (props.job.score >= 75) return '#10b981'
  if (props.job.score >= 45) return '#f59e0b'
  return '#ef4444'
})
...
<template>
  <div class="job-card" @click="$emit('click', job)" style="position: relative;">
    ...
    <!-- Bottom-right: status badge if locked, action buttons otherwise -->
    <div style="position: absolute; bottom: 1.2rem; right: 1.2rem; display: flex; gap: 0.5rem; align-items: center; z-index: 10;">

      <!-- LOCKED: show status badge -->
      <template v-if="isLocked">
        <ApplicationStatusBadge :status="appStatusRaw" size="sm" @click.stop />
      </template>

      <!-- FREE: show action icons -->
      <template v-else>
        <button
          @click.stop="toggleSave()"
          style="background: white; border: 1px solid var(--border-gray); border-radius: 50%; width: 32px; height: 32px; display: flex; align-items: center; justify-content: center; transition: all 0.2s; cursor: pointer;"
          :style="{ borderColor: isSaved ? '#ef4444' : 'var(--border-gray)' }"
          title="Save Job"
        >
        ...
        >
          <span style="color: var(--text-light); font-size: 1.1rem; line-height: 1;">📤</span>
        </button>
      </template>
    </div>
  </div>
